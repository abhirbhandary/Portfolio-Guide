...
| 🧹 [Data Cleaning & Storytelling](https://github.com/abhirbhandary/Python/blob/main/Data_Cleaning_with_Pandas.ipynb) | Data Wrangling & ETL | I have shown basic data cleaning steps that should be performed on all new datasets where necessary. Applied Pareto principle states that for many outcomes, roughly 80% of consequences come from 20% of causes (the "vital few"). Applying this principle we extract only Apple and Refurbished Apple data from the dataset. Since the company is based on apple products and its accesiories we consider this data. | pandas |   
<!--
